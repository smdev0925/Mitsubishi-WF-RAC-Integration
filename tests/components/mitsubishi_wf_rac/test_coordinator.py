...
async def test_the_retry_waits_out_what_is_left_of_the_lock(
    ...
    with patch(
        "custom_components.mitsubishi_wf_rac.coordinator.asyncio.sleep"
    ) as sleep:
        await hass.services.async_call(
            CLIMATE_DOMAIN,
            SERVICE_SET_FAN_MODE,
...
async def test_an_answered_command_counts_as_proof_of_life(
    ...
    with patch("custom_components.mitsubishi_wf_rac.coordinator.asyncio.sleep"):
        await hass.services.async_call(
            CLIMATE_DOMAIN,
            SERVICE_SET_FAN_MODE,
...
async def test_a_poll_that_never_answers_counts_as_a_missed_poll(
    ...
    with patch(
        "custom_components.mitsubishi_wf_rac.coordinator.POLL_TIMEOUT",
        timedelta(seconds=0),
    ):
